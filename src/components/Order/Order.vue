<template>
  <div class="container-fluid mb-5 mt-5 border">
    <div class="container-fluid mt-4 mb-4">
      ...
      <div v-for="(menu, index) in order" :key="menu.name" class="">
        <div class="row">
          ...
          <div class="col-md-3 row">
            ...
            <div class="col-md-6">
              {{Math.round(menu.price*100)/100}}元
            </div>
          </div>
          <div class="col-md-2 text-right mt">
            <div class="money text-danger">
              ￥{{Math.round(menu.total*100)/100}}
            </div>
          </div>
        </div>
        <hr>
...
export default {
  ...
  computed: {
    ...
    urls: function () {
      let urls = []
      for (let menu of this.order) {
        if (Object.keys(menu).length !== 0 && menu.picture != null) {
          let url = '/api/images/' + menu.picture
          urls.push(url)
          continue
        }
        urls.push('./')
